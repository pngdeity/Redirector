...
async function saveChanges() {
	...
	const response = await chrome.runtime.sendMessage({type:"save-redirects", redirects:arr});
...
async function toggleSyncSetting() {
	const response = await chrome.runtime.sendMessage({type:"toggle-sync", isSyncEnabled: !options.isSyncEnabled});
    if(response.message === "sync-enabled"){
        ...
        options.isSyncEnabled = false;
        await chrome.storage.local.set({isSyncEnabled: options.isSyncEnabled});
        showMessage(response.message, false);
    }
    else {
